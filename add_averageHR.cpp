...
#define BODY_TYPE 3
#define AVERAGE_ELEMENT_NUM 300
...
int main(){
    fstream excel;
    ofstream avrData;
    excel.open("A_30_12.csv");
    avrData.open("AverageHR.csv");
    ...
    while (getline(excel, lineEx))
    {
        if(lineEx[0]>='0' && lineEx[0]<='9'){
            ...
            // }
            tempIndex++;
            senHRBuff.buff[senHRBuff.tail]=stoi(tempVector[2]);
        ...
        // else{
        //     avrData<<lineEx<<","<<"Avr Sen HR,Avr Real HR"<<'\n';
        // }
